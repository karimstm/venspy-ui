...
export const fetch_projects = () => async dispatch => {
  const response = await axios.get("/project/").catch(() =>
    dispatch({
      type: PROJECTS_FAILED,
      payload: "Failed to retrieve list of projects"
    })
  );
...
export const fetch_models = project_id => async dispatch => {
  const response = await axios.get(`/project/${project_id}/models`).catch(() =>
    dispatch({
      type: MODELS_FETCH_FAILURE,
...
export const fetchTypes = () => async dispatch => {
  const response = await axios
    .get("/types/")
    .catch(() =>
      dispatch({ type: TYPES_FETCH_FAILURE, payload: "Could not fetch types" })
    );
...
export const uploadModel = data => async dispatch => {
  const response = await axios.post("/upload/", data).catch(err => {
    return dispatch({
      type: MODEL_INSERT_FAILURE,
...
export const deleteModel = modelId => async dispatch => {
  const response = await axios.delete(`/upload/${modelId}`).catch(e => {
    console.log(e.response);
    return dispatch({
      type: MODEL_DELETE_FAILURE,
      payload: "Failed to DELETE File"
    });
  });
  if (response.status === 204)
    return dispatch({ type: MODEL_DELETE_SUCCESS, payload: response.data });
  return response;
};
