...
class ProjectList extends Component {
  ...
  componentDidMount() {
    this.setState({ isLoading: true }, async () => {
      ...
      if (response !== undefined) this.setState({ isLoading: false });
  ...
  renderList = () => {
    ...
    return projects.map(
      ({ id, name, description, modification_date, creation_date, runs }) => {
        return (
          <Col key={id} className="gutter-row" span={8}>
            <ProjectCard
              ...
              run={runs}
              id={id}
              modification_date={this.checkLastModification(
                creation_date,
                modification_date
              )}
  ...
  render() {
    return (
      <div>
        ...
        <Row gutter={16}>{this.renderList()}</Row>
      </div>
    );
  }
}
