...
// analysing the name to find graphs
        ...
        if (name_parts.length > 1)
            this.componentType = name_parts[1];
...
// small verification of element coordinates
        ...
        if (this.w === null || this.h === null || this.x === null || this.y === null || this.shape == null
            || isNaN(this.w) || isNaN(this.h) || isNaN(this.x) || isNaN(this.y) || isNaN(this.shape))
        {
            if (leetDebugMode === true)
                console.log("the following object has unvalid coordinates or shape : ", parsedObject);
        }
...
// loading the rest of the properties
        ...
        this.textPosRelative = get_value_or_default(parsedObject.textPosRelative, 0);
...
// Loading colors
        ...
        this.wordColor = get_color_or_default(get_color_or_default(parsedObject.fontColor, parsedObject.boxColor), parentViewClass.fontColor);
        this.fillColor = get_color_or_default(parsedObject.fillColor, parentViewClass.backgroundColor);

// More processing to simplify things

        this.hovered = false;
        this.fontSize = parentViewClass.fontSize;
        let text_offset = [[0,0], [0, this.h], [-this.w, 0], [0, -this.h], [this.w, 0]];
        ...
        this.textBox = [this.x + current_offset[0], this.y + current_offset[1], this.w, this.h];
    }

    draw()
    {
        // debug mode with all background white and borders black
        if (leetDebugDrawMode)
    ...
    alternative_draw_mode()
    {
        ...
        if (this.hovered && this.isClickable)
            fill(0x45, 0x45, 0x45);
        else
            fill(0x2c, 0x2c, 0x2c);
        ...
        if ((this.textPosRelative <= 0 || this.textPosRelative > 4) && 2 * this.fontSize > this.h)
        {
            textAlign(CENTER);
    ...
    draw_icon()
    {
        ...
        if (this.componentType == 'Graph')
            image(leetComponentImages['Graph'], this.x, this.y + 3*this.h/4 - this.fontSize, this.w/4, this.h/4);
        fill(0x2c, 0x2c, 0x2c);
        ...
        {
            rect(this.x, this.y + this.h - this.fontSize - 2, this.w, this.fontSize + 4);
            fill(0xe5, 0xe5, 0xe5);
            text(this.name, this.x, this.y + this.h - this.fontSize, this.w, this.fontSize + 4);
        }
        pop();
    }
    ...
    click()
    {
        if (this.isClickable && !isNaN(this.clickTargetView) && this.clickTargetView != 0)
